...
CREATE procedure [Reporting].[TwinspiresPurchasesPopulate]
--declare
...
CREATE TABLE #TransactionsInPeriod
(
	Id BIGINT
...
INSERT INTO #TransactionsInPeriod
SELECT
	t.Id
...
select
	...
	,convert(varchar(60),cast(ch.UnitPrice as decimal (18,2))) as [Actual Price Charged]
	,'' as [Reason for Price Difference]
	,ch.InvoiceId
from 
	customer c
	inner join #TransactionsInPeriod t on t.CustomerId = c.id
...
from 
	...
	inner join #TransactionsInPeriod rt on rch.Id = rt.Id
	INNER JOIN CustomerReference cr ON c.Id = cr.Id
	LEFT JOIN SalesTrackingCode stc ON stc.Id = cr.SalesTrackingCode1Id	
...
select
	''''+c.Reference as [CAM ID]
	,c.Id as FusebillId
	,''  as [Subscription Product ID]
    ,''  as [Purchase ID]
    ,ISNULL(stc.Code,'') as [Affiliate ID]
	,convert(varchar(60),convert(smalldatetime,dbo.fn_GetTimezoneTime (wot.effectiveTimestamp,@Timezoneid))) as [Transaction Date]
	,wot.Id as [Transaction ID]
	,'' as [Product code]
	,'' as PlanName
	,'' as Reference
	,convert(varchar(60),cast(-wot.Amount as decimal (18,2)) ) as [Regular Price]
	,convert(varchar(60),cast(-wot.Amount as decimal (18,2))) as [Actual Price Charged]
	,'WO' as [Reason for Price Difference]
	,wo.InvoiceId
FROM
	WriteOff wo
	inner join #TransactionsInPeriod wot on wot.Id = wo.Id
	inner join Customer c on c.Id = wot.CustomerId
	INNER JOIN CustomerReference cr ON wot.CustomerId = cr.Id
	LEFT JOIN SalesTrackingCode stc ON stc.Id = cr.SalesTrackingCode1Id	
	where wot.TransactionTypeId = 10 
...
Select pn.InvoiceId, 
	...
	inner join Invoice i ON pn.InvoiceId = i.Id 
	inner join #Results r	on i.CustomerId = r.FusebillId
where 
	i.AccountId = @AccountId
	--No date filtering as Twinspires always cares about what the invoice was paid by
group by pn.InvoiceId,pm.AccountType
...
From 
	...
	inner join #TransactionsInPeriod T	on ch.id = t.id 
...
from 
	...
	on clj.CustomerId = r.CustomerId
where clj.effectivetimestamp < @EndDate
Group by clj.CustomerId
...
--populate semi-permanent reporting table
insert into Reporting.Twinspires_DailyActivityDetails
select
	[CAM ID] 	
	...
	,COALESCE(CAST(ccb.[Current Account Balance] as MONEY),0) as [Current Account Balance]
...
from #Results r
	...
	left join @CurrentCustomerBalance ccb	on r.FusebillId = ccb.Id
	left join @CustomerGroups cg on cg.FusebillId = r.FusebillId

drop table #Results
drop table #Credits
drop table #Refunds
DROP TABLE #PaymentMethods
DROP TABLE #TransactionsInPeriod
set nocount off
