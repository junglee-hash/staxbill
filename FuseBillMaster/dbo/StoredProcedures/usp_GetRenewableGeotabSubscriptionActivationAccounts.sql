CREATE   PROCEDURE [dbo].[usp_GetRenewableGeotabSubscriptionActivationAccounts]
...
BEGIN
	...
	WHERE agc.StatusId = 1
		...
		AND agc.SubscriptionActivation = 1
		AND agc.SubscriptionActivationNextRunTimestamp <= @RunDate
		AND NOT EXISTS (
			SELECT *
			FROM AccountAutomatedHistory ah
			WHERE agc.Id = ah.AccountId
				AND ah.AccountAutomatedHistoryTypeId = 22
				AND (
