...
CREATE   PROCEDURE [dbo].[usp_GetFullPlansByCode]
	@Codes nvarchar(max),
	@AccountId bigint
AS
BEGIN
...
INSERT INTO @planCodes (PlanCode)
select Data from dbo.Split (@Codes,'|')

INSERT INTO @plans
SELECT Id FROM [Plan] p
INNER JOIN @planCodes ON p.Code = PlanCode AND AccountId = @AccountId
WHERE p.IsDeleted = 0
...
INSERT INTO @OrderToCashCycleIds SELECT pocc.Id
	...
	INNER JOIN @plans plans ON pr.PlanId = plans.PlanId
...
FROM [Plan] p
INNER JOIN @plans plans ON p.Id = plans.PlanId

SELECT * FROM PlanRevision pr
INNER JOIN @plans plans ON pr.PlanId = plans.PlanId AND pr.IsActive = 1
