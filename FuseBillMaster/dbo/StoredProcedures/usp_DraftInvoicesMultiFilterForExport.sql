CREATE PROCEDURE [dbo].[usp_DraftInvoicesMultiFilterForExport]   
 --DECLARE  
 @AccountId bigint,   
...
select cd.*,  ditemp.PoNumber as [PO Number], ditemp.BillingPeriodStartDate,ditemp.BillingPeriodEndDate, ditemp.PostingDate as [Estimated Posting Date]  
	, ditemp.DueDate as [Estimated Due Date], ditemp.TermName as [Net Terms], ditemp.status, ditemp.Amount, ditemp.SumOfCharges,   
	ditemp.SumOfTaxes as [Total Taxes],  ditemp.SumOfDiscounts as [Total Discounts], ditemp.Currency, ditemp.ID as [Draft invoice ID], ditemp.AvalaraId as [Avalara ID], FORMAT(ditemp.ReferenceDate, 'MM/dd/yyyy') as [Invoice Reference Date]
