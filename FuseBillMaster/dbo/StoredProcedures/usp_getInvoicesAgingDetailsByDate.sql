CREATE   PROCEDURE [dbo].[usp_getInvoicesAgingDetailsByDate]  
 --required
	@AccountId BIGINT = 19 
...
--We do not need terminal journals for invoices with one payment schedule
--We need all statuses for invoices with many payment schedules so we can get the numbering right
...
, bc.* 
FROM #ResultSet rs 
INNER JOIN BasicCustomerDataByAccount(@AccountId) bc ON bc.[Fusebill ID] = rs.CustomerId
ORDER BY bc.[Fusebill ID], [Transaction Type], [Posted Timestamp]  
