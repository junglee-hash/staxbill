...
CREATE   PROCEDURE [dbo].[usp_GetDebitsForSageIntacctSync]
AS
BEGIN
	...
	INNER JOIN Debit dr ON dr.Id = t.Id
		AND dr.SageIntacctId IS NULL
		AND dr.SageIntacctAttemptNumber < 5
	INNER JOIN Credit cr on cr.Id = dr.OriginalCreditId
		AND cr.SageIntacctId IS NOT NULL
