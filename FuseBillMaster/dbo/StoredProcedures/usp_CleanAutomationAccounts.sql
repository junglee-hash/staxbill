...
BEGIN TRY
	...
	if  @AccountId = 0
		   ...
		   SELECT TOP (@NumberToDelete) AccountId from
		   (
	...
	UPDATE a SET
		...
		, [HubSpotConfigured] = 0
		, [SageIntacctEnabled] = 0
	FROM AccountFeatureConfiguration a
	INNER JOIN #IntegrationAccountList i ON a.Id = i.AccountId

	-- EXCLUDE FROM BILLING
	UPDATE a
	SET a.IncludeInAutomatedProcesses = 0
		,a.ShutdownDate = GETUTCDATE()
		,a.ShutdownReason = 'Excluding integration test'
	FROM Account a
	INNER JOIN #IntegrationAccountList i ON a.Id = i.AccountId
	...
	-- EXCLUDE FROM EARNING
		...
		DELETE
			targetTable
		FROM [dbo].[AccountLogoLog] targetTable
			INNER JOIN #IntegrationAccountList i
				ON targetTable.AccountId = i.AccountId
		...
		DELETE
			targetTable
		FROM [dbo].[HubspotUser] targetTable
			INNER JOIN #IntegrationAccountList i
				ON targetTable.AccountId = i.AccountId

		DELETE
			targetTable
		FROM [dbo].[HubspotClerkAccess] targetTable
			INNER JOIN #IntegrationAccountList i
				ON targetTable.Id = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
			   AccountHubspotCouponCodeMapping targetTable
			   inner join #IntegrationAccountList i
			   on targetTable.AccountHubSpotConfigurationId = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
			   AccountSageIntacctConfiguration targetTable
			   inner join #IntegrationAccountList i
			   on targetTable.Id = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
				ExternalPaymentMethod targetTable
				inner join PaymentMethod pm ON targetTable.PaymentMethodId = pm.Id
		...
		delete 
				targetTable 
		from 
				[dbo].[HostedPageManagedSectionStatement] targetTable
		...
		DELETE
			targetTable
		FROM
			AccountGeotabDevicePlan targetTable
			INNER JOIN #IntegrationAccountList i
			ON targetTable.AccountId = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
			   InvoiceChildTaxDocument targetTable
		...
		DELETE 
			   targetTable
		FROM 
			   CustomerEmailEventSummary   targetTable
			   inner join #IntegrationAccountList i
			   on targetTable.AccountId = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
			   PaymentNoteAttempt targetTable
			   inner join Invoice p
			   on targetTable.InvoiceId = p.id
			   inner join #IntegrationAccountList i
			   on p.AccountId = i.AccountId
		...
		FROM 
			   ...
			   inner join Invoice ii
			   on targetTable.InvoiceId = ii.Id
			   inner join #IntegrationAccountList i
			   on ii.AccountId = i.AccountId
		...
		FROM 
			   ...
			   inner join Invoice ii
			   on ii.Id = targetTable.InvoiceId
			   inner join #IntegrationAccountList i
			   on ii.AccountId = i.AccountId
		...
		FROM
			   PaymentNoteAttempt targetTable
			   inner join PaymentActivityJournal  paj ON paj.Id = targetTable.PaymentActivityJournalId
			   inner join #IntegrationAccountList i
			   on paj.AccountId = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
			   AvalaraConfigurationBySalesTrackingCode targetTable
			   inner join #IntegrationAccountList i
			   on targetTable.AvalaraConfigurationId = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
			   AnrokConfiguration targetTable
			   inner join #IntegrationAccountList i
			   on targetTable.Id = i.AccountId
		...
		DELETE 
			   targetTable
		FROM 
			   [HostedPageManagedAvailableCountry]  targetTable
			   inner join [HostedPageManagedSelfServicePortal] hpm
		...
		DELETE 
			   targetTable
		FROM 
			   HostedPageManagedSectionPaymentMethodLabel  targetTable
			   inner join HostedPageManagedSelfServicePortal hpm
			   on targetTable.HostedPageManagedSelfServicePortalId = hpm.id 
			   inner join HostedPage hp
			   on hpm.HostedPageId = hp.id 
		...
		DELETE 
			   targetTable
		FROM 
			   HostedPageManagedSectionProfile  targetTable
			   inner join HostedPageManagedSelfServicePortal hpm
			   on targetTable.HostedPageManagedSelfServicePortalId = hpm.id 
			   inner join HostedPage hp
			   on hpm.HostedPageId = hp.id 
		...
		DELETE 
			   targetTable
		FROM 
			   HostedPageManagedSectionLabel  targetTable
			   inner join HostedPageManagedSelfServicePortal hpm
			   on targetTable.id = hpm.id 
			   inner join HostedPage hp
			   on hpm.HostedPageId = hp.id 
		...
		DELETE 
			   targetTable
		FROM 
			   SubscriptionGeotabDevice  targetTable
			   inner join Subscription s
			   on s.Id = targetTable.Id 
			   inner join #IntegrationAccountList i
	...
	BEGIN TRAN;
		...
		DELETE
			targetTable
		FROM Price targetTable
		INNER JOIN QuantityRange qr ON qr.Id = targetTable.QuantityRangeId
		INNER JOIN OrderToCashCycle otc ON otc.Id = qr.OrderToCashCycleId
		INNER JOIN PricebookEntry pe ON otc.Id = pe.OrderToCashCycleId
		INNER JOIN Pricebook p ON p.Id = pe.PricebookId
		inner join #IntegrationAccountList i
			   on p.AccountId = i.AccountId

		DELETE
			targetTable
		FROM QuantityRange targetTable
		INNER JOIN OrderToCashCycle otc ON otc.Id = targetTable.OrderToCashCycleId
		INNER JOIN PricebookEntry pe ON otc.Id = pe.OrderToCashCycleId
		INNER JOIN Pricebook p ON p.Id = pe.PricebookId
		inner join #IntegrationAccountList i
			   on p.AccountId = i.AccountId
		...
		DELETE
			targetTable
		Output deleted.OrderToCashCycleId into @OrdertoCashCycle
		FROM PricebookEntry targetTable
		INNER JOIN Pricebook p ON p.Id = targetTable.PricebookId
		inner join #IntegrationAccountList i
			   on p.AccountId = i.AccountId

		DELETE
			targetTable
		FROM PricebookMaxPrice targetTable
		INNER JOIN Pricebook p ON p.Id = targetTable.PricebookId
		inner join #IntegrationAccountList i
			   on p.AccountId = i.AccountId
