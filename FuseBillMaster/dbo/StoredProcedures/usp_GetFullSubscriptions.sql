...
CREATE        PROCEDURE [dbo].[usp_GetFullSubscriptions]
	@subscriptionIds AS dbo.IDList READONLY,
	@includeDeletedTrackedItems bit,
	@accountId bigint
WITH RECOMPILE
AS
BEGIN
	...
	if @includeDeletedTrackedItems = null
	begin
		set @includeDeletedTrackedItems = 1;
	end
...
SELECT distinct 
	...
	WHERE (@includeDeletedTrackedItems = 0 AND i.StatusId = 1) OR (@includeDeletedTrackedItems = 1)
	AND EXISTS (
		SELECT 1
		FROM @Products p
...
union
	SELECT bp.*
		,bp.[PeriodStatusId] as PeriodStatus
	FROM [dbo].[BillingPeriod] bp
	INNER JOIN SubscriptionProductCharge spc on bp.Id = spc.BillingPeriodId
...
SELECT
  ...
  SELECT * FROM SubscriptionProductCharge spc
    INNER JOIN @subscriptionproducts sp ON sp.SubscriptionProductId = spc.SubscriptionProductId
