...
CREATE   PROCEDURE [dbo].[usp_ARBalanceReportCSV]
@AccountId BIGINT
,@CurrencyId BIGINT
,@StartDate DATETIME
,@EndDate DATETIME
...
BEGIN
	...
	----Filtered transaction by input parameters
	SELECT
	t.Id
	...
	WHERE  t.AccountId = @AccountId 
	AND t.CurrencyId = @CurrencyId 
	AND pn.EffectiveTimestamp >= @StartDate 
	AND pn.EffectiveTimestamp < @EndDate
	GROUP BY pn.InvoiceId, pn.PaymentId
	...
	FROM (
		...
		FROM ( 
			...
			,CONVERT(SMALLDATETIME,dbo.fn_GetTimezoneTime(t.EffectiveTimestamp,@TimezoneId)) AS [Posted Date]
			,CASE
			...
			AND t.Id IN (SELECT Id FROM #FilteredTransactions)
			) [Inner]
...
-----------------------
DROP TABLE #FilteredTransactions
